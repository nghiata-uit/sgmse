...
# pylint: skip-file
# NCSN++ model with Mamba architecture
...
@BackboneRegistry.register("ncsnpp_mamba")
class NCSNppMamba(nn.Module):
    """NCSN++ model with Mamba architecture"""
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        modules.append(MambaConv(channels, nf))
        hs_c = [nf]
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                modules.append(MambaResBlock(in_ch=in_ch, out_ch=out_ch, temb_dim=nf * 4))
                in_ch = out_ch
                hs_c.append(in_ch)
        ...
        in_ch = hs_c[-1]
        modules.append(MambaBottleneck(channels=in_ch, temb_dim=nf * 4))
        ...
        for i_level in reversed(range(num_resolutions)):
            for i_block in range(num_res_blocks + 1):  # +1 blocks in upsampling because of skip connection from combiner (after downsampling)
                out_ch = nf * ch_mult[i_level]
                modules.append(MambaResBlock(in_ch=in_ch + hs_c.pop(), out_ch=out_ch, temb_dim=nf * 4))
                in_ch = out_ch
    ...
    def forward(self, x, time_cond):
        ...
        h = modules[m_idx](h, temb)  # Mamba bottleneck
        m_idx += 1
